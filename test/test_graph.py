...
class GraphSetupMethodsTestCase(TestCase):
    ...
    def test_add_edge(self):
        ...
        self.graph.add_edge(source_node_name='a', destination_node_name='b', edge_weight=3)
        graph = self.graph._Graph__graph
    ...
    def test_add_edges(self):
        self.graph._Graph__graph = {'a': [], 'b': [], 'c' :[]}
        self.graph.add_edge(source_node_name='a', destination_node_name='b', edge_weight=3)
        self.graph.add_edge(source_node_name='a', destination_node_name='c', edge_weight=7)
        self.graph.add_edge(source_node_name='b', destination_node_name='c', edge_weight=2)
        self.graph.add_edge(source_node_name='c', destination_node_name='b', edge_weight=4)
        graph = self.graph._Graph__graph
    ...
    def test_add_repititive_edges(self):
        self.graph._Graph__graph = {'a': [], 'b': [], 'c' :[]}
        self.graph.add_edge(source_node_name='a', destination_node_name='b', edge_weight=3)
        self.graph.add_edge(source_node_name='b', destination_node_name='c', edge_weight=7)
        self.graph.add_edge(source_node_name='a', destination_node_name='b', edge_weight=2)
        self.graph.add_edge(source_node_name='a', destination_node_name='b', edge_weight=4)
        graph = self.graph._Graph__graph
    ...
    def test_add_edge_with_not_existing_src_node(self):
        self.graph._Graph__graph = {'a': [], 'b': [], 'c' :[]}
        self.graph.add_edge(source_node_name='d', destination_node_name='b', edge_weight=3)
        graph = self.graph._Graph__graph
        expected_graph = {'a': [], 'b': [], 'c' :[]}
        self.assertEqual(graph, expected_graph)
    ...
    def test_add_edge_with_not_existing_dst_node(self):
        self.graph._Graph__graph = {'a': [], 'b': [], 'c' :[]}
        self.graph.add_edge(source_node_name='a', destination_node_name='d', edge_weight=3)
        graph = self.graph._Graph__graph
        expected_graph = {'a': [], 'b': [], 'c' :[]}
        self.assertEqual(graph, expected_graph)
    ...
    def test_add_edge_with_not_existing_both_of_nodes(self):
        self.graph._Graph__graph = {'a': [], 'b': [], 'c' :[]}
        self.graph.add_edge(source_node_name='i', destination_node_name='j', edge_weight=3)
        graph = self.graph._Graph__graph
        expected_graph = {'a': [], 'b': [], 'c' :[]}
        self.assertEqual(graph, expected_graph)
    ...
    def test_add_edge_with_zero_weight(self):
        self.graph._Graph__graph = {'a': [], 'b': [], 'c' :[]}
        self.graph.add_edge(source_node_name='a', destination_node_name='b', edge_weight=0)
        graph = self.graph._Graph__graph
        expected_graph = {'a': [], 'b': [], 'c' :[]}
        self.assertEqual(graph, expected_graph)
...
class GraphQueryTestCase(TestCase):
    ...
    def test_get_all_pathes_emtpy(self, mocked_get_not_weighted_graph):
        graph = Graph()
        graph.add_node('a')
        graph.add_node('b')
        pathes = graph.get_all_paths('a', 'b')
        self.assertEqual(pathes, [])
        mocked_get_not_weighted_graph.assert_called_once()
    
    @mock.patch.object(Graph, '_Graph__get_not_weighted_graph', return_value={'a': ['b'], 'b': []})
    def test_get_all_pathes_one_path(self, mocked_get_not_weighted_graph):
        graph = Graph()
        graph.add_node('a')
        graph.add_node('b')
    ...
    @mock.patch.object(Graph, '_Graph__get_not_weighted_graph', return_value={'a': ['b', 'c'], 'b': ['c'], 'c':[]})
    def test_get_all_pathes_multiple_pathes(self, mocked_get_not_weighted_graph):
        graph = Graph()
        graph.add_node('a')
        graph.add_node('b')
        graph.add_node('c')
...
class GraphPrivateFunctionsTestCase(TestCase):
    ...
    def  test_get_not_weighted_graph_empty(self):
        graph = Graph()
        not_weighted_graph = graph._Graph__get_not_weighted_graph()
    ...
    @mock.patch.object(Graph, '_Graph__get_path_weight', return_value='__mocked_weight__')
    def test_get_all_paths(self, mocked_get_path_weight):
        graph = Graph()
        self.__make_full_graph(graph)
    ...
    @mock.patch.object(Graph, '_Graph__get_path_weight', return_value='__mocked_weight__')
    def test_get_path_weight_called_correctly_in_get_all_paths(self, mocked_get_path_weight):
        graph = Graph()
        self.__make_full_graph(graph)
        visited = {k: False for k in graph._Graph__nodes}
        graph.graph = {'0': ['1', '2', '3'],
        ...
        get_path_weight_calls = [call.args[0] for call in mocked_get_path_weight.call_args_list]
        self.assertTrue(['0', '1', '3'] in get_path_weight_calls)
        self.assertTrue(['0', '2', '1', '3'] in get_path_weight_calls)
        self.assertTrue(['0', '3'] in get_path_weight_calls)
    
    def test_get_path_weight(self):
        graph = Graph()
        self.__make_full_graph(graph)
        path_weight = graph._Graph__get_path_weight(['0', '1', '3'])
        expected_path_weight = 1 + 8
        self.assertEqual(path_weight, expected_path_weight)
